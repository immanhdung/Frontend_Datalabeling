  ...
  const fetchProjects = async () => {
    try {
      ...
      const enhanced = (Array.isArray(data) ? data : []).map(p => {
        ...
        const projectTasks = allLocalTasks.filter(t => String(t.projectId) === pid);
        ...
        return {
          ...
          completionRate: totalTasks > 0 ? Math.round((approvedCount / totalTasks) * 100) : 0
        };
      });
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap gap-4">
               ...
               <div className="bg-indigo-50 border border-indigo-100 px-6 py-4 rounded-3xl">
                  <p className="text-indigo-600 text-[10px] font-black uppercase mb-1">Tổng Task đạt chuẩn</p>
                  <p className="text-3xl font-black text-indigo-700">{projects.reduce((acc, p) => acc + p.approvedTasks, 0)}</p>
               </div>
            </div>
          </div>
        </div>
