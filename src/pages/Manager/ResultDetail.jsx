      ...
      for (const task of projectTasks) {
        ...
        try {
          ...
          taskItems.forEach(item => {
            ...
            const itemAnns = taskAnns.filter(a => String(a.itemId || a.item_id) === String(itemId));
            
            aggregated.push({
               ...item,
               taskTitle: task.title || task.name,
               taskId: taskId,
               annotations: itemAnns,
               allLabels: Array.from(new Set(itemAnns.flatMap(a => (a.payload?.bboxes || []).map(b => b.label || b.labelName)))),
               classification: itemAnns.find(a => a.classification)?.classification || ''
            });
          });
        } catch (err) {
        ...
        <div className="max-w-screen-2xl mx-auto flex items-center justify-between gap-6">
           ...
           <div className="flex items-center gap-4">
              <div className="hidden md:flex items-center bg-slate-100 rounded-2xl p-1 border border-slate-200">
                 <button className="px-4 py-2 bg-white rounded-xl shadow-sm text-xs font-black text-slate-900">GALLERY</button>
                 <button className="px-4 py-2 text-xs font-black text-slate-400 hover:text-slate-600">TABLE</button>
              </div>
                ...
                {filteredItems.map((item, idx) => (
                   ...
                   >
                      <div className="relative aspect-[4/3] bg-slate-100 overflow-hidden">
                         <img 
                           src={item.imageUrl || item.url || (item.datasetItem?.imageUrl)} 
         ...
         <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-8 lg:p-12 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="w-full h-full max-w-7xl flex flex-col lg:flex-row gap-8 lg:gap-12">
               ...
               <div className="flex-1 bg-black/40 rounded-[3rem] border border-white/5 relative overflow-hidden flex items-center justify-center">
                  <img 
                    src={selectedItem.imageUrl || selectedItem.url || (selectedItem.datasetItem?.imageUrl)} 
                    className="max-w-full max-h-full object-contain"
                    alt="preview"
                  />
                  {/* Overlay Labels (Mock visualization or just info) */}
                  <div className="absolute bottom-10 left-10 right-10 flex flex-wrap gap-3">
                     {selectedItem.allLabels.map((l, i) => (
                        <div key={i} className="bg-white px-4 py-2 rounded-2xl shadow-2xl flex items-center gap-3">
                           <div className={`w-3 h-3 rounded-full bg-indigo-500`} />
                           <span className="text-sm font-black text-slate-900 uppercase">{l}</span>
                        </div>
                     ))}
                  </div>
               </div>
                  ...
                  <div className="space-y-4">
                     <div className="bg-white/5 border border-white/10 rounded-3xl p-6">
                        ...
                        <div className="space-y-3">
                           {(selectedItem.annotations[0]?.payload?.bboxes || []).map((b, i) => (
                              <div key={i} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
                                 <span className="text-xs font-bold text-white/80 uppercase">{b.label}</span>
                                 <span className="text-[10px] text-white/30 font-mono">[{Math.round(b.x)}, {Math.round(b.y)}, {Math.round(b.width)}, {Math.round(b.height)}]</span>
                              </div>
                           ))}
                           {(!selectedItem.annotations[0]?.payload?.bboxes || selectedItem.annotations[0].payload.bboxes.length === 0) && (
                              <p className="text-xs text-white/20 italic">Không có dữ liệu bounding box.</p>
                           )}
                        </div>
                     </div>
